    ...
    for (let r = 0; r < sudoku.size; r++) {
        for (let c = 0; c < sudoku.size; c++) {
            if (
                ...
                sudoku.grid[r][c].candidates.length == 0
            ) {
                return true
            }
        }
...
export function update_candidates(sudoku) {
    for (let r = 0; r < sudoku.size; r++) {
        for (let c = 0; c < sudoku.size; c++) {
            ...
            sudoku.grid[r][c].candidates = sudoku.grid[r][c].candidates.filter(
                (cand) => {
                    // remove if in row
...
export function grade(sudoku) {
    ...
    while (!is_solved(currentSudoku)) {
        ...
        if (is_unsolvable(currentSudoku)) {
            ...
            const nextCandidate = cell.candidates.find(
                (c) => !last.triedCandidates.includes(c)
            )
        ...
        history.push({
            // we push this sudoku into history to retrieve it with pop later if our guess turns out to be incorrect
    ...
    return Math.round(gradeScore * 100) / 100
}
